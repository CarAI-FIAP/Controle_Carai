...
void Prints(){
  #if EXIST_DADOS
  dados_print_PC += String(inicio); // estado geral dos motores. (ligado = 1 desligado = 0)
  dados_print_PC += ",";
...
dados_print_PC += ",";
  ...
  // dados_print_PC += " ";
  dados_print_PC += String(0);  // angulo de inclinação do carro  String(angulo_z_f); 
  dados_print_PC += ",";
  ...
  // dados_print_PC += ",";
  // dados_print_PC += String(0); // angulo que o servo recebe da visão
  // dados_print_PC += " ";
  ...
  dados_print_PC += String(obstaculo); // informa a deteção do utrass. do meio (detectado = 1, não detectado = 0)
  dados_print_PC += ",";
